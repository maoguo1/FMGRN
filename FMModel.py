...
def subcode(X_train, X_test, b, minf, maxf):
    F = len(X_train[0])
    X_train_o = np.zeros((X_train.shape[0], F*b))
    X_test_o = np.zeros((X_test.shape[0], F*b))
    a = []
    p = []
...
def FMpredict(data_x, data_y,b):
    minf_X,maxf_X,train_x1 = prenormal(data_x)
    X_train, X_test, y_train, y_test = train_test_split(train_x1, data_y, test_size=0.2, random_state=0)
    X_train_o, X_test_o = subcode(X_train, X_test, b,minf_X,maxf_X)
    ...
    fm.fit(train_X, y_train)
    y_pred_train = fm.predict(train_X)
    y_pred = fm.predict(test_X)
    test_MSE = np.sum((y_pred - y_test) ** 2) / len(y_test)
    train_MSE = np.sum((y_pred_train - y_train) ** 2) / len(y_train)
...
def get_importances(TS_data, time_points, alpha="from_data",  SS_data=None, gene_names=None,regulators='all', b=1):
    ...
    for i in range(ngenes):
        ...
        if i in input_idx:
            input_idx.remove(i)
        vi = get_importances_single(TS_data, time_points, alphas[i], input_idx, i, SS_data, b)
        VIM[i,:] = vi
...
def get_importances_single(TS_data, time_points, alpha, input_idx, output_idx, SS_data, b):
    ...
    w,v =FMpredict(input_all, output_all,  b)
    #vi = getvar_w(w, ngenes, b)
    vi = getMAXMIN_w(w, ngenes, b)
    #vi = getabsmax_w(w, ngenes, b)
    #vi = getabssum_w(w, ngenes, b)
...
def get_scores(VIM, gold_edges, gene_names, regulators):
    ...
    #print("fpr",fpr,"thresholds",thresholds )
